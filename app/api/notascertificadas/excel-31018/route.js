...
const YEAR_COLUMNS = [
  { nombre: 1, numero: 4, letras: 5, te: 9, mes: 10, anio: 11, plantel: 13 }, // 4to
  { nombre: 1, numero: 4, letras: 5, te: 9, mes: 10, anio: 11, plantel: 13 }  // 5to
];

const YEAR_START_ROWS = [21, 38];
const YEAR_ROW_COUNTS = [11, 10];
const ALLOWED_GRADES = new Set(['4', '5']);
...
const TEMPLATE_PATH = path.join(process.cwd(), 'public', 'Formato31018.xlsx');
...
export async function POST(request) {
  try {
    ...
    if (!fs.existsSync(TEMPLATE_PATH)) {
      return NextResponse.json({ success: false, message: 'Plantilla Formato31018.xlsx no encontrada en /public' }, { status: 404 });
    }
    ...
    console.log('📊 Plan de estudio FINAL a procesar (formato 31018):', JSON.stringify(planFiltrado, null, 2));
    ...
    ws.getCell('H8').value = institucionData.entidadFederal || '';
    ...
    (planFiltrado || []).forEach((anio) => {
      const grado = String(anio?.grado || '');
      const idx = grado === '4' ? 0 : 1;
    ...
    return new NextResponse(buffer, {
      ...
      headers: {
        ...
        'Content-Disposition': `attachment; filename=Formato31018_${notaEst.cedula || 'estudiante'}.xlsx`
      }
    });
  } catch (error) {
    console.error('POST /api/notascertificadas/excel-31018 error', error);
